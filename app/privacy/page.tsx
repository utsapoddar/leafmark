      ...
      <article className="privacy-document">
        ...
        <div className="privacy-sections">
          ...
          <section>
            ...
            <p>When you test a connection or use an AI model, the browser sends the key to the provider or custom endpoint you selected so that service can authenticate the request. NVIDIA is the exception: its API blocks direct browser requests, so NVIDIA keys pass transiently through Leafmark&apos;s relay. Leafmark does not store or retain them. Use a dedicated, revocable key with a spending limit whenever your provider supports those controls.</p>
          </section>
          ...
          <section>
            <h2>4. NVIDIA relay</h2>
            <p>When the built-in NVIDIA connection is enabled and you select it, your API key, bounded book excerpts, model settings, and ordinary network metadata pass through a Leafmark-operated Cloudflare Worker before reaching NVIDIA. The relay has no application API key, database, cache, analytics code, or intentional request-content logging. It holds each bounded request only long enough to forward it and does not store it.</p>
            <p>Cloudflare necessarily processes the transient request while operating the relay under its <a href="https://www.cloudflare.com/privacypolicy/" target="_blank" rel="noreferrer">Privacy Policy</a>. If you do not want to use Leafmark&apos;s relay, connect a local NIM or a relay you control under Custom.</p>
          </section>

          <section>
            <h2>5. Custom endpoints</h2>
          ...
          <section>
            <h2>6. Hosting, cookies, and analytics</h2>
